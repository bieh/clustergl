    ...
    for ( yloop = 0; yloop < pyramidSize; yloop++ )
	{
	    for ( xloop = 0; xloop < pyramidSize; xloop++ )
		{
		    ...
		    glTranslatef( -14.0f*multiplier + 2.8f * xloop + 1.4f*odd,
				  -14.0f*multiplier + 2.8f * yloop + 1.4f,
				  -36.0f*multiplier);

		    /* Tilt the cubes */
		    /* Tilt The Cubes Up And Down */
		    glRotatef( 45.0f - ( 6.0f ),
			       1.0f, 0.0f, 0.0f );
			...
			if(retro)
				...
				randCol = yloop%5;
		    glColor3fv( boxcol[randCol] ); /* Select A Box Color */
    ...
    while ( !done )
	{
	    ...
	    while ( SDL_PollEvent( &event ) )
		{
			...
			case SDL_ACTIVEEVENT:
				...
				isActive = TRUE; /* always draw it (i.e. don't need to be active) */
			    else
				isActive = TRUE;
			    break;			    
