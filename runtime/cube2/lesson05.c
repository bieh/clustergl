...
int LoadGLTextures( )
{
	...
	//colour cube
	//vs = "varying float xpos;varying float ypos;varying float zpos;void main(void){xpos = clamp(gl_Vertex.x,0.0,1.0);ypos = clamp(gl_Vertex.y,0.0,1.0);zpos = clamp(gl_Vertex.z,0.0,1.0);gl_Position = gl_ModelViewProjectionMatrix * gl_Vertex;}";
	//fs = "varying float xpos;varying float ypos;varying float zpos;void main (void){gl_FragColor = vec4 (xpos +0.1, ypos +0.1, zpos +0.1, 1.0);}";
	...
	const char * vv = vs;
	const char * ff = fs;
	GLint size = strlen((char *)vv);
	GLint *length = &size;
	GLint size2 = strlen((char *)ff);
	GLint *length2 = &size2;
	/* link shader string to shader objects */
	glShaderSource(v, 1, &vv, length);
	...
	glCompileShader(f);
	glGetShaderiv(f, GL_COMPILE_STATUS, &compiled2);
	fflush(stdout);
