...
void sendPointers(int length)
{
	...
	//texture pointer
								 //check if sent already, and not null
	if(!rpTex.sent && rpTex.size) {
		pushOp(320);
...
{
	LOG("Called broken stub TexParameterfv!\n");
	pushOp(179);
	pushParam(target);
	pushParam(pname);
...
{
	LOG("Called broken stub TexParameteriv!\n");
	pushOp(181);
	pushParam(target);
	pushParam(pname);
...
{
	LOG("Called broken stub TexEnvfv!\n");
	pushOp(185);
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLfloat) * sizeof(params));
...
{
	LOG("Called broken stub TexEnviv!\n");
	pushOp(187);
	pushParam(target);
	pushParam(pname);
...
{
	LOG("Called broken stub TexGendv!\n");
	pushOp(189);
	pushParam(coord);
	pushParam(pname);
...
extern "C" void glTexGenfv(GLenum coord, GLenum pname, const GLfloat * params)
{
	...
	pushParam(pname);
	int size = sizeof(const GLfloat);
	if (pname == GL_EYE_PLANE || pname == GL_OBJECT_PLANE)
		size *= 4;
	pushBuf(params, size);
}
...
{
	LOG("Called broken stub GetBooleanv!\n");
	pushOp(258);
	pushParam(pname);
	int size = sizeof(GLboolean); //needs to * by size. List of pname is huge!
...
{
	LOG("Called broken stub GetPixelMapfv!\n");
	pushOp(271);
	pushParam(map);
...
{
	LOG("Called broken stub GetPixelMapuiv!\n");
	pushOp(272);
	pushParam(map);
...
{
	LOG("Called broken stub GetPixelMapusv!\n");
	pushOp(273);
	pushParam(map);
...
extern "C" const GLubyte * glGetString(GLenum name)
{
	...
	const GLubyte * ret = (GLubyte *)malloc(sizeof(GLubyte *) * MAX_STRING_LENGTH);
	pushBuf(ret, sizeof(GLubyte *) * MAX_STRING_LENGTH, true);
	waitForReturn();
	return ret;
}
...
{
	LOG("Called untested stub GetTexEnvfv!\n");
	pushOp(276);
	pushParam(target);
	pushParam(pname);
	int size = sizeof(GLfloat);
...
{
	LOG("Called untested stub GetTexGendv!\n");
	pushOp(278);
	pushParam(coord);
	pushParam(pname);
	int size = sizeof(GLdouble) * 4;
	//all size 4 minus GL_TEXTURE_GEN_MODE
...
{
	LOG("Called untested stub GetTexGenfv!\n");
	pushOp(279);
	pushParam(coord);
	pushParam(pname);
...
{
	LOG("Called untested stub GetTexGeniv!\n");
	pushOp(280);
	pushParam(coord);
	pushParam(pname);
	int size = sizeof(GLint) * 4;
	//all size 4 minus GL_TEXTURE_GEN_MODE
	if(pname == GL_TEXTURE_GEN_MODE)
	{
	size = sizeof(GLint);
...
{
	LOG("Called untested stub GetTexParameterfv!\n");
	pushOp(282);
	pushParam(target);
	pushParam(pname);
	int size = sizeof(GLfloat);
...
{
	LOG("Called untested stub GetTexParameteriv!\n");
	pushOp(283);
	pushParam(target);
	pushParam(pname);
	int size = sizeof(GLint);
...
{
	LOG("Called unimplemted stub glFrustum!\n");
	LOG("fix me!\n");
...
{
	LOG("Called broken stub IndexPointer!\n");
	pushOp(314);
	pushParam(type);
	pushParam(stride);
	int arraySize = 1;			 //TODO: use pointer structure
...
extern "C" void glGetColorTableParameterfv(GLenum target, GLenum pname, GLfloat * params)
{
	...
	pushParam(pname);
	int size = sizeof(GLfloat);
	if(target == GL_COLOR_TABLE || target == GL_POST_CONVOLUTION_COLOR_TABLE || target == GL_POST_COLOR_MATRIX_COLOR_TABLE)
	{
	size *= 4;
	}
	pushBuf(params, size, true);
...
extern "C" void glGetHistogramParameterfv(GLenum target, GLenum pname, GLfloat * params)
{
	...
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLfloat), true);
	waitForReturn();
}
...
extern "C" void glGetMinmax(GLenum target, GLboolean reset, GLenum format, GLenum type, GLvoid * values)
{
	...
	pushParam(format);
	pushParam(type);
	pushBuf(values, getFormatSize(format) * getTypeSize(type) * 2, true);
...
extern "C" void glLoadTransposeMatrixf(const GLfloat * m)
{
	...
	pushOp(408);
								 //4x4 matrix
	pushBuf(m, sizeof(const GLfloat) * 16, true);
	waitForReturn();
}
...
extern "C" void glLoadTransposeMatrixd(const GLdouble * m)
{
	...
	pushOp(409);
								 //4x4 matrix
	pushBuf(m, sizeof(const GLdouble) * 16, true);
	waitForReturn();
}
...
{
	LOG("Called untested stub GetCompressedTexImage!\n");
	pushOp(419);
	pushParam(target);
	pushParam(level);
...
{
	LOG("Called broken stub SecondaryColorPointer!\n");
	pushOp(448);
	pushParam(size);
	pushParam(type);
	pushParam(stride);
...
extern "C" void glGetAttachedShaders(GLuint program, GLsizei maxCount, GLsizei * count, GLuint * obj)
{
	...
	pushParam(program);
	pushParam(maxCount);
	pushBuf(count, sizeof(GLsizei));
	pushBuf(obj, sizeof(GLuint) * maxCount, true);
	waitForReturn();
}
...
{
	LOG("Called untested stub GetAttribLocation!\n");
	pushOp(502);
	pushParam(program);
	pushBuf(name, sizeof(GLchar) * MAX_STRING_LENGTH, true);
	waitForReturn();
}
...
{
	LOG("Called untested stub GetProgramiv!\n");
	pushOp(503);
	pushParam(program);
	pushParam(pname);
	pushBuf(params, sizeof(GLint), true);
...
{
	LOG("Called untested stub GetProgramInfoLog!\n");
	pushOp(504);
	pushParam(program);
	pushParam(bufSize);
	pushBuf(length, sizeof(GLsizei));
...
extern "C" void glUniformMatrix2x3fv(GLint location, GLsizei count, GLboolean transpose, const GLfloat * value)
{
	...
	pushParam(transpose);
								 //2x3
	pushBuf(value, sizeof(GLint) * count * 6);
}
...
extern "C" void glUniformMatrix2x4fv(GLint location, GLsizei count, GLboolean transpose, const GLfloat * value)
{
	...
	pushParam(transpose);
								 //2x4
	pushBuf(value, sizeof(GLint) * count * 8);
}
...
extern "C" void glUniformMatrix3x4fv(GLint location, GLsizei count, GLboolean transpose, const GLfloat * value)
{
	...
	pushParam(transpose);
								 //3x4
	pushBuf(value, sizeof(GLint) * count * 12);
}
...
{
	LOG("Called untested stub GetProgramStringARB!\n");
	pushOp(722);
	pushParam(target);
	pushParam(pname);
	pushBuf(string, sizeof(GLchar) * MAX_STRING_LENGTH, true);
	waitForReturn();
}
...
{
	LOG("Called untested stub GetHistogramParameterfvEXT!\n");
	pushOp(800);
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLfloat), true);
...
{
	LOG("Called untested stub GetHistogramParameterivEXT!\n");
	pushOp(801);
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLint), true);
...
{
	LOG("Called untested stub GetMinmaxEXT!\n");
	pushOp(802);
	pushParam(target);
	pushParam(reset);
	pushParam(format);
...
{
	LOG("Called untested stub GetMinmaxParameterfvEXT!\n");
	pushOp(803);
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLfloat) * 2, true);
...
{
	LOG("Called untested stub GetMinmaxParameterivEXT!\n");
	pushOp(804);
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLint) * 2, true);
...
{
	LOG("Called untested stub GetConvolutionParameterfvEXT!\n");
	pushOp(818);
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLfloat) * sizeof(params), true);
...
{
	LOG("Called untested stub GetConvolutionParameterivEXT!\n");
	pushOp(819);
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLint) * sizeof(params), true);
...
extern "C" void glGetDetailTexFuncSGIS(GLenum target, GLfloat * points)
{
	...
	pushOp(844);
	pushParam(target);
	pushBuf(points, sizeof(GLfloat), true);	//need size
	waitForReturn();
}
...
{
	LOG("Called untested stub GetSharpenTexFuncSGIS!\n");
	pushOp(846);
	pushParam(target);
	pushBuf(points, sizeof(GLfloat), true); //need size
	waitForReturn();
...
extern "C" void glSpriteParameterfvSGIX(GLenum pname, const GLfloat * params)
{
	...
	pushOp(860);
	pushParam(pname);
	pushBuf(params, sizeof(GLfloat)); //need size
}
...
extern "C" void glSpriteParameterivSGIX(GLenum pname, const GLint * params)
{
	...
	pushOp(862);
	pushParam(pname);
	pushBuf(params, sizeof(GLint)); //need size
}
...
extern "C" GLint glPollInstrumentsSGIX(GLint * marker_p)
{
	...
	pushOp(867);
	pushBuf(marker_p, sizeof(GLint)); //need size
}
...
extern "C" void glGetFogFuncSGIS(GLfloat * points)
{
	...
	pushOp(876);
	pushBuf(points, sizeof(GLfloat)); //need size
}
...
extern "C" void glGetImageTransformParameterivHP(GLenum target, GLenum pname, GLint * params)
{
	...
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLint), true); //need size
	waitForReturn();
}
...
extern "C" void glGetImageTransformParameterfvHP(GLenum target, GLenum pname, GLfloat * params)
{
	...
	pushParam(target);
	pushParam(pname);
	pushBuf(params, sizeof(GLfloat), true); //need size
	waitForReturn();
}
...
extern "C" void glCullParameterdvEXT(GLenum pname, GLdouble * params)
{
	...
	pushOp(900);
	pushParam(pname);
	pushBuf(params, sizeof(GLdouble)); //need size
}
...
extern "C" void glFragmentMaterialivSGIX(GLenum face, GLenum pname, const GLint * params)
{
	...
	pushParam(face);
	pushParam(pname);
	pushBuf(params, sizeof(const GLint)); //need size
}
...
extern "C" GLint glFinishAsyncSGIX(GLuint * markerp)
{
	...
	pushOp(925);
	pushBuf(markerp, sizeof(GLuint)); //need size
	//returns a value
}
...
extern "C" void glReplacementCodeuivSUN(const GLuint * code)
{
	...
	pushOp(998);
	pushBuf(code, sizeof(GLuint)); //need size
}
...
extern "C" void glReplacementCodeusvSUN(const GLushort * code)
{
	...
	pushOp(999);
	pushBuf(code, sizeof(GLushort)); //need size
}
...
extern "C" void glReplacementCodeubvSUN(const GLubyte * code)
{
	...
	pushOp(1000);
	pushBuf(code, sizeof(GLubyte)); //need size
}
...
{
	LOG("Called untested stub Color4ubVertex2fvSUN!\n");
	pushOp(1003);
	pushBuf(c, sizeof(GLubyte) * 4);
	pushBuf(v, sizeof(GLfloat) * 2);
}
...
{
	LOG("Called untested stub Color4ubVertex3fvSUN!\n");
	pushOp(1005);
	pushBuf(c, sizeof(GLubyte) * 4);
	pushBuf(v, sizeof(GLfloat) * 3);
}
...
{
	LOG("Called untested stub Color3fVertex3fvSUN!\n");
	pushOp(1007);
	pushBuf(c, sizeof(GLfloat) * 3);
	pushBuf(v, sizeof(GLfloat) * 3);
}
...
{
	LOG("Called untested stub Normal3fVertex3fvSUN!\n");
	pushOp(1003);
	pushBuf(n, sizeof(GLfloat) * 3);
	pushBuf(v, sizeof(GLfloat) * 3);
}
...
{
	LOG("Called untested stub Color4fNormal3fVertex3fvSUN!\n");
	pushOp(1003);
	pushBuf(c, sizeof(GLfloat) * 4);
	pushBuf(n, sizeof(GLfloat) * 3);
	pushBuf(v, sizeof(GLfloat) * 3);
}
...
{
	LOG("Called untested stub TexCoord2fVertex3fvSUN!\n");
	pushOp(1013);
	pushBuf(tc, sizeof(GLfloat) * 2);
	pushBuf(v, sizeof(GLfloat) * 3);
}
...
{
	LOG("Called untested stub TexCoord4fVertex4fvSUN!\n");
	pushOp(1015);
	pushBuf(tc, sizeof(GLfloat) * 4);
	pushBuf(v, sizeof(GLfloat) * 4);
}
...
{
	LOG("Called untested stub TexCoord2fColor4ubVertex3fvSUN!\n");
	pushOp(1017);
	pushBuf(tc, sizeof(GLfloat) * 2);
	pushBuf(c, sizeof(GLubyte) * 4);
	pushBuf(v, sizeof(GLfloat) * 3);
...
{
	LOG("Called untested stub TexCoord2fColor3fVertex3fvSUN!\n");
	pushOp(1019);
	pushBuf(tc, sizeof(GLfloat) * 2);
	pushBuf(c, sizeof(GLfloat) * 3);
	pushBuf(v, sizeof(GLfloat) * 3);
...
{
	LOG("Called untested stub TexCoord2fNormal3fVertex3fvSUN!\n");
	pushOp(1021);
	pushBuf(tc, sizeof(GLfloat) * 2);
	pushBuf(n, sizeof(GLfloat) * 3);
	pushBuf(v, sizeof(GLfloat) * 3);
...
{
	LOG("Called untested stub ReplacementCodeuiVertex3fvSUN!\n");
	pushOp(1027);
	pushBuf(rc, sizeof(GLuint));
	pushBuf(v, sizeof(GLfloat) * 3);
}
...
{
	LOG("Called untested stub ReplacementCodeuiColor4ubVertex3fvSUN!\n");
	pushOp(1029);
	pushBuf(rc, sizeof(GLuint));
	pushBuf(c, sizeof(GLubyte) * 4);
	pushBuf(v, sizeof(GLfloat) * 3);
...
{
	LOG("Called untested stub ReplacementCodeuiColor3fVertex3fvSUN!\n");
	pushOp(1031);
	pushBuf(rc, sizeof(GLuint));
	pushBuf(c, sizeof(GLfloat) * 3);
	pushBuf(v, sizeof(GLfloat) * 3);
...
{
	LOG("Called untested stub ReplacementCodeuiNormal3fVertex3fvSUN!\n");
	pushOp(1029);
	pushBuf(rc, sizeof(GLuint));
	pushBuf(n, sizeof(GLfloat) * 3);
	pushBuf(v, sizeof(GLfloat) * 3);
...
{
	LOG("Called untested stub ReplacementCodeuiTexCoord2fVertex3fvSUN!\n");
	pushOp(1037);
	pushBuf(rc, sizeof(GLuint));
	pushBuf(tc, sizeof(GLfloat) * 2);
	pushBuf(v, sizeof(GLfloat) * 3);
...
extern "C" void glGetProgramStringNV(GLuint id, GLenum pname, GLubyte * program)
{
	...
	pushParam(id);
	pushParam(pname);
	pushBuf(program, sizeof(GLchar) * MAX_STRING_LENGTH, true);
	waitForReturn();
}
...
extern "C" void glProgramParameter4dvNV(GLenum target, GLuint index, const GLdouble * params)
{
	...
	pushParam(target);
	pushParam(index);
	pushBuf(params, sizeof(const GLdouble) * 4, true);
	waitForReturn();
}
...
extern "C" void glProgramParameter4fvNV(GLenum target, GLuint index, const GLfloat * params)
{
	...
	pushParam(target);
	pushParam(index);
	pushBuf(params, sizeof(const GLfloat) * 4, true);
	waitForReturn();
}
...
extern "C" void glSetFragmentShaderConstantATI(GLuint dst, const GLfloat * value)
{
	...
	pushOp(1195);
	pushParam(dst);
	pushBuf(value, sizeof(GLfloat)); //need size
}
